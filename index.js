function initGame() {
  ...
  const showGameOver = () => {
    ...
    // Show restart button under game over, hide main restart button
    let restartGameOver = document.getElementById("restart-gameover");
    if (!restartGameOver) {
      restartGameOver = document.createElement("button");
      restartGameOver.id = "restart-gameover";
      restartGameOver.textContent = "Restart";
      restartGameOver.addEventListener("click", restartGame);
      document.body.appendChild(restartGameOver);
    }
    restartGameOver.style.display = "block";
    restartEl.style.display = "none"; // Hide the main restart button
  ...
  const restartGame = () => {
    ...
    const overText = document.getElementById("game_over");
    if (overText) overText.remove();
    const restartGameOver = document.getElementById("restart-gameover");
    if (restartGameOver) restartGameOver.style.display = "none";

    // Hide the main restart button
